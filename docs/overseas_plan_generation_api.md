...
| `file_path` | 实际生成文件路径。 |
| `metadata.report_version` | 客户版/内部版。 |
| `metadata.audit_log_path` | `word_export_audit_log.jsonl` 导出记录路径。 |
...
| `file_path` | 实际生成文件路径。 |
| `metadata.export_kind` | `action_plan` 或 `resource_list`。 |
...
| `system_name` | string | 否 | 页脚/导出记录展示的生成机构/系统名称，默认“企业出海方案智能生成系统”。 |
| `report_version` | string | 否 | `client` 或 `internal`；客户版用于对外沟通，内部版会保留内部评审提示。 |
| `logo_text` | string | 否 | 页眉右上角展示的 Logo/品牌文字；未传时默认使用企业名称。 |
| `theme_color` | string | 否 | 主题色，支持 `#005BAC` 或 `005BAC` 等 6 位十六进制颜色。 |
| `footer_text` | string | 否 | 每页页脚文字；未传时使用系统名、版本和企业名称。 |
...
{
  "project_id": "ogp_2f5e...",
  "plan_name": "示例医疗科技出海客户汇报稿",
  "export_type": "PPT",
  "file_path": "/tmp/agent_overseas_report/exports/ppt/ogp_2f5e.../示例医疗科技出海客户汇报稿_客户版_v1_20260507080000.pptx",
  "exported_by": "user-1001",
  "exported_at": "2026-05-07T08:00:00Z",
  "report_version": "client",
  "slide_count": 20,
  "audit_log_path": "/tmp/agent_overseas_report/exports/ppt/ogp_2f5e.../ppt_export_audit_log.jsonl"
}
```

### PPT 内容结构

导出 PPT 标题固定为《`{企业名称}出海客户汇报稿`》，默认生成 20 页咨询风格宽屏演示稿；每页标题均为观点式结论句，避免大段正文，优先使用表格、评分卡、矩阵和路线图：

1. 封面：以优先国家为突破口的汇报结论。
2. 核心结论：优先市场、进入模式、能力短板和 12-24 个月目标。
3. 企业出海机会判断：需求、供给、渠道、政策四类机会。
4. 企业与产品画像：企业基础、核心产品、认证资质和卖点。
5. 出海成熟度评分：维度得分和补强建议。
6. 目标国家优先级矩阵：市场潜力、进入难度、优先级和进入模式。
7. 目标市场规模与增长：国家/区域规模、增长和数据复核动作。
8. 竞品与价格带：竞品判断与我方打法。
9. 客户与渠道结构：经销商、KA、服务商、线上线索推进逻辑。
10. 推荐进入模式：经销代理、展会获客、KA 试点、本地化评估。
11. 渠道推进路径：长名单、短名单、试销、区域合作。
12. 展会/推介会/采购对接计划：活动安排、目标对象和产出指标。
13. 海外资源匹配：渠道、认证技术、供应链和公共资源。
14. 合规与政策风险：认证准入、关税政策、售后合同和回款约束。
15. 融资与扩产路径：初期、中期、后期资金/产能触发条件。
16. 预算与 KPI：线索、渠道、合规、交付四类预算与复盘指标。
17. 12个月路线图：准备、验证、样板订单和渠道体系里程碑。
18. 24个月路线图：区域复制、服务网络、本地化和经营闭环。
19. 关键风险与应对：红黄绿灯机制、责任人和止损条件。
20. 下一步行动建议：30 天内市场复核、渠道长名单和首轮访谈。

PPT 使用可配置主题色、Logo/品牌文字、页脚、微软雅黑中文字体声明，并统一表格、矩阵和时间轴表格样式；文件写入系统可访问路径后，项目的 `output_ppt.file_path` 会保存该路径。
...
| `file_path` | 实际生成文件路径。 |
| `metadata.report_version` | 客户版/内部版。 |
| `metadata.slide_count` | 默认 20 页。 |
| `metadata.audit_log_path` | `ppt_export_audit_log.jsonl` 导出记录路径。 |
